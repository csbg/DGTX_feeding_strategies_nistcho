...
growth <- df_mu_summary %>%
  ggplot(aes(x = Hours, y = mean_mu*24, color = Condition)) +
  geom_line(linewidth = 0.6, na.rm = FALSE) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymin = (mean_mu - se_mu) * 24, ymax = (mean_mu + se_mu) * 24, color = Condition), width = 3, na.rm = TRUE) +
  labs(
  ...
  scale_y_continuous(limits = c(-2.6, 1), breaks = seq(-2.5, 1, 0.5))
